...
class _FakeActionTrace:
    def __init__(self):
        self.chunks = []
        self.executions = []

    def record_action_chunk(self, *, timesteps, actions):
        self.chunks.append({"timesteps": timesteps, "actions": actions})

    def record_execution(self, *, timestep, queue_len, executed):
...
def test_robot_client_records_async_action_trace(robot_client):
    trace = _FakeActionTrace()
    robot_client.action_trace = trace

    incoming = _make_actions(start_ts=time.time(), start_t=10, count=2)
    robot_client._record_received_action_chunk(incoming)

    assert trace.chunks[0]["timesteps"] == [10, 11]
    assert torch.equal(trace.chunks[0]["actions"][0], incoming[0].get_action())

    robot_client.action_queue.put(incoming[0])
    performed = robot_client.control_loop_action()

    assert trace.executions[0]["timestep"] == 10
    assert trace.executions[0]["queue_len"] == 1
    assert trace.executions[0]["executed"] == performed
