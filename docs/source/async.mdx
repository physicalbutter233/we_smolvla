...
python -m lerobot.async_inference.robot_client \
    ...
    --debug_visualize_queue_size=True \ # CLIENT: visualize the queue size after the client exits
    --display_action_trace=true \ # CLIENT: show queue size, predicted chunks, and executed actions in real time
    --display_data=true # CLIENT: show observations, including all camera streams, in Rerun
```
...
- `display_action_trace` opens a live, Rerun-independent Matplotlib window. Its top plot is the action queue size; each action dimension shows the original predicted chunk as a dashed line and the action actually sent to the robot as a solid line.
- `display_data` initializes the Rerun viewer and logs the client observations, including every configured camera image.
